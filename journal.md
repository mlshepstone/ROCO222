...
we connected servo to an arduino, with a program that generated movement at frequency of about 0.2 Hz. The program we used was:
...
```
The code steps from 0-180 then back again with each step accuring every 13.9 ms. we used 13.9 ms as this makes the servo follow a sine wave witha frequency of around 0.2 Hz. This was calculated using the following maths: 
...
The servo the needed to be controlled using a potentiometer. The code we used for this was:
...
```
To scale the value of the potentiometer that was recived to a value that would turn servo a correspodingamount, I divided the collected potentiometer by 5.683. 5.683 was calculated by dividing 1023 (the maximum value of the potentiometer) by 180 (the maximum value of the servo), resulting in~ 5.683 There is also some code that reads the value of the potentiometer which we later comented out, Whilst the code was uncommented it was used to collect these values: 

![Task 6 step 2](https://github.com/Schenkington/Images/blob/master/task%206%20step%202.png)

3. A robot arm mock-up

we constructed a test robot atm using 2 servo motors and soem cardboard the resultign arm looked like this.

![Mock arm](https://github.com/Schenkington/Images/blob/master/Mock%20arm.JPG)

We were meant to use the mock up arm to estimate the torque of the servo motor however we ended up using ice lolly sticks with centimeter marks on them as we had issues keeping the mas stable on the arm.

![Torque Measure](https://github.com/Schenkington/Images/blob/master/Torque%20measure.JPG)

when tested with this method the motor stalled when a 65g weight was ~4cm away from the turning point of the motor, meaning the motor has a a tourque of 0.26 kgfcm as calculated below

65g = 0.065 kg
T = 4*0.065 
T = 0.26 kgfcm

This is much less than the 1.6 kgfcm that is the stall torque stated in the datasheet.

The mock arm required 2 servos each controlled by a potentiometer. The code we wrote for this was:
